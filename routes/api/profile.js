...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // The same as push, but unshift put data on top
...
router.get('/github/:username', (req, res) => {
  try {
    // Connecting to GitHub API for users repositories
    const options = {
      // per one page 5 repositories => per_page=5
      // sort repositories by creation date ascending => sort=create:asc
    ...
    request(options, (error, response, body) => {
      ...
      if (response.statusCode !== 200) {
        return res.status(404).json({ msg: 'No GitHub pofile found' });
      }
